ig.module('game.main')
.requires('impact.game', 'impact.image', 'impact.sound')
.defines(function() {
    Firework = ig.Game.extend({
        width: 0,
        height: 0,
        bits: 5000,
        bitPx: [], bitPy: [], bitVx: [], bitVy: [],
        bitSx: [], bitSy: [], bitL: [], bitF: [],
        bitW: [], bitC: [], bitMax: 150,
        mx: 50, my: 50,
        pixelsData: [],
        soundOn: true,
        ...
        update: function() {
            if (ig.input.pressed('shoot') || ig.input.pressed('shoot2')) {
                ig.music.play();
                if (ig.input.mouse.y > this.height) return;
                ...
                for (var i = 0; i < this.bits; i++) {
                    ...
                    this.bitF[i] = 2;
                    if (++count === this.bitMax) break;
                }
                if (this.soundOn) this.shootSound.play();
            }
        },
        ...
        rend: function() {
            ...
            if (explode && this.soundOn) this.explodeSound.play();
        },
